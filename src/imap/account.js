...
var properties = {
  ...
  _bindConnectionDeathHandlers: function(conn) {
    ...
    conn.onerror = function(err) {
      err = imaperrors.normalizeImapError(conn, err);
  ...
  _syncFolderComputeDeltas: function(conn, callback, err, boxesRoot) {
    ...
    function walkBoxes(boxLevel, pathDepth, parentId) {
      boxLevel.forEach(function(box) {
        ...
        // XXX: Browserbox includes "empty-named" mailboxes in the
        // hierarchy for some reason. The old imap.js silently
        // discarded the leading path separator. With browserbox, for
        // the IMAP fake-server, we end up with the following
        // structure:
        //
        // root
        //  |- "inbox"
        ...
        //     |- "/Trash"
        //
        // However, the fakeserver does not recognize "/Sent" as a valid
        // mailbox path...
        if (!boxName && box.children) {
          walkBoxes(box.children, pathDepth);
          return;
        }
